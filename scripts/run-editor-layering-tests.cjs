...
const coffee = pantry.addFurniture('custom-coffee', 0, 48, 64, 64, 96)
const coffeeTable = pantry.addFurniture('custom-coffee-table', 16, 48, 112, 64, 32)
pantry.addFurniture('custom-pantry-front-chair', 12, 48, 160, 64, 64)
click(pantry, 'custom-coffee-table')
assert.ok(coffee.depth > coffeeTable.depth, 'coffee machine stays above its table even when the table was selected last')
assert.ok(coffee.depth < depth(pantry, 'custom-pantry-front-chair'), 'coffee machine does not jump in front of nearer furniture')
assert.ok(pantry.collisionRects().some(rect => rect.x === coffee.x - 34 && rect.y === coffee.y - 50),
  'coffee machine remains a solid obstacle despite rendering on the tabletop')
pantry.setLayoutEditing(false)
pantry.refreshFurnitureDepths()
assert.ok(coffee.depth > coffeeTable.depth, 'finishing editing retains the coffee machine above its table')
pantry.setLayoutEditing(true)
coffeeTable.events.get('drag')(pointer, 176, 112)
assert.ok(coffee.depth < coffeeTable.depth, 'separating the machine and table restores normal floor ordering')
console.log('PASS coffee machine above its table, solid collision, edit completion and separation')
