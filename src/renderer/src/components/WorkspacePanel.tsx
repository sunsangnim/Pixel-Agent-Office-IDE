...
const CATEGORIES = [
  { path: WORKSPACE_FOLDERS.required, label: '필수 파일', description: '에셋 · 애니메이션 · 기존 작업' },
  ...
  return (
    <dialog ref={dialog} className="workspace-panel" onCancel={(e) => { e.preventDefault(); onClose() }} aria-labelledby="workspace-title">
      <header className="workspace-header">
        <div><h2 id="workspace-title">작업실 파일</h2><p>필수 파일과 산출물을 나누어 관리하세요.</p></div>
      ...
      <footer className="workspace-footer">필수 파일 · 오피스 리소스　 |　 산출물 · 의뢰한 작업 결과 <span>5초마다 갱신</span></footer>
    </dialog>
  )
}
