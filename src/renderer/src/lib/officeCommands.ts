...
const RETURN = /^(?:(?:자기|각자|네|너희|본인)?자리(?:로)?)(?:돌아가(?:줘요?|주세요|요)?|가(?:줘요?|주세요|세요|라)?|복귀(?:해(?:줘요?|주세요|요)?|하자|하세요)?)$/

export interface OfficeCommand { action: 'visit' | 'return'; templateIds: string[] }
...
export function parseOfficeCommand(text: string): OfficeCommand | null {
  ...
  return action ? { action, templateIds: [...new Set(ids)] } : null
}
