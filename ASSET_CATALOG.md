...
- `characters/ceo-desk-work-v3.png` — 대표 전용 투명 2×2 업무 자세 시트.
  왼쪽 위부터 정면·왼쪽·뒷면·오른쪽이며, 양손을 책상 높이로 올리고 팔꿈치를 굽힌 자세다.
  내장 이미지 생성 도구로 기존 외형과 착석 위치를 유지하고 손을 키보드 쪽으로 더 뻗도록 제작했다.
  `ceo-desk-work-v1.png`은 이전 자세 비교용 원본이며 런타임에서는 사용하지 않는다.
- 기존 착석 시트의 배율과 좌표 변환을 그대로 적용한다. 앞으로 뻗은 손이 생겨도
  캐릭터 전체를 다시 가운데 정렬하거나 축소하지 않는다. 업무 프레임의 좌우에
  동일한 여백을 더해 384×360으로 렌더링하므로 길게 뻗은 손끝도 잘리지 않는다.
- `representativeWorkAnimation.ts`가 손·소매에 작은 타이핑 움직임과 짧은 휴식을 넣는다.
  팔을 늘리거나 어깨를 잘라 붙이지 않고, 전체 자세의 투명도와 외곽선을 유지한다.
  회의석과 독립 의자는 기존 휴식 자세를 사용한다.

## Employee desk work posture

- `characters/work-v1/staff-{team}-work-v1.png` — 팀 0~2의 직원 15종 업무 자세.
  기존 `seated-v1` 외형과 승인된 대표 자세를 참조해 내장 이미지 생성 도구로 제작한 투명 PNG다.
  5열은 기존 외형 순서, 3행은 정면·뒷면·왼쪽이며 오른쪽은 왼쪽을 반전한다.
- `staffWorkAnimation.ts`가 기존 캐릭터 배율과 발 위치를 유지해 의자에 맞춘다.
  실제 투명 간격으로 열을 분리해 길게 뻗은 손끝과 옆 캐릭터가 섞이지 않게 한다.
  384×360 프레임에서 대표와 같은 손가락 움직임을 사용하며 모든 투명도·외곽선은 유지한다.
- 자기 책상에 도착한 직원만 타이핑하며 캐릭터별로 박자를 다르게 재생한다.
  이동·휴식·회의·도움 요청·오류에는 기존 자세를 사용하고 편집 중에는 시간이 멈춘다.
  기존 Claude 팀장 외형 선택, 클릭 선택, 이름과 가구의 겹침 순서는 유지한다.
